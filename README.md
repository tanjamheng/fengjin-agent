...
3. **8GB 以上内存** — 有 NVIDIA 显卡可自动启用 GPU 加速（见下方显存清单）；无独显则 CPU 运行
4. **预留 12GB 磁盘空间** — 模型下载与量化、Python 环境、前端依赖、向量库缓存都会占用空间
5. **一个 API Key** — 任何 OpenAI 兼容协议的都可以（智谱、DeepSeek、硅基流动、OpenAI 官方等）

#### GPU 加速（自动检测，无需手动配置）

启动脚本会自动检测 NVIDIA 显卡（`nvidia-smi`）：

- **有独显** → 自动装 CUDA 版 PyTorch（~2.5GB）→ 嵌入和检索走 GPU → 首次构建知识库 **~15 秒**
- **无独显**（核显/AMD/苹果芯片）→ CPU 版 PyTorch（~200MB）→ 首次构建知识库 **~4 分钟**

配置文件已统一设 `device: "auto"`（`config/rag.yaml` + `config/memory.yaml`），Runtime 自动匹配。

| 场景 | 加载模型 | 显存占用 |
|------|---------|:------:|
| 构建知识库（仅首次） | bge-m3 | **~2.0 GB** |
| 日常对话（RAG 检索） | bge-m3 + bge-reranker-v2-m3 | **~2.8 GB** |
| PyTorch GPU 缓存池 | — | **~0.5-1.0 GB** |
| **首个会话峰值** | 以上全部 | **~3.5 GB** |

> ⚠️ 有 GPU 时，bge-m3 和 reranker 会话期间一直占用显存，无GPU时为内存，关掉应用才释放。PyTorch 缓存池在模型释放时自动回收（`torch.cuda.empty_cache()`），但首个会话加载后峰值较高是正常的。
> ⚠️ 有 GPU 时，bge-m3 和 reranker 会话期间一直占用显存，无GPU时为内存，关掉应用才释放。
...
双击即开，全程零命令行。左边角色展示区始终显示，右边加载区展示两阶段进度：**预处理**（自动扫描缺失模型→下载→FP16 量化→构建知识库，每步有进度反馈，15 分钟无响应弹重试）→ **系统加载**（初始化安全/记忆/情绪/羁绊/角色漂移/RAG 六大引擎，5 分钟无响应弹重试）。出错可重试/跳过。加载完成淡入聊天界面。有 NVIDIA 显卡自动 GPU 加速。
