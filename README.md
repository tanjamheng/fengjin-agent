...
4. **预留 8GB 磁盘空间** — 模型下载约 2.2GB + Python 环境约 2GB + 前端依赖约 0.5GB
5. **一个 API Key** — 任何 OpenAI 兼容协议的都可以（智谱、DeepSeek、硅基流动、OpenAI 官方等，免费额度也能跑）

> ⏳ **首次启动需要等待约 5-10 分钟**：脚本会自动创建虚拟环境、安装 Python 依赖（~2GB）和前端依赖、下载本地模型（~2.2GB）并量化。后续启动只需几秒。Windows 启动脚本已配置国内镜像源加速下载。
...
1. **首次运行**：加载页自动扫描缺失模型 → 联网下载（约 3GB）→ FP16 量化 → 构建知识库 → 初始化引擎，全程进度条动画。如果还没配 API Key，会弹出设置面板引导填写。
...
cd frontend && npm run dev       # 前端（Electron 窗口）
```
