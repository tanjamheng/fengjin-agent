...
- 前端角色展示图及头像等素材来源于《崩坏：星穹铁道》官方公开视觉内容，用于同人项目的角色展示。
- `数据侧_风堇资料` 是 RAG 知识库导入目录，包含角色设定、剧情对话、语音、背景设定等资料，用于提升风堇还原度。release 包会包含该目录；缺失时程序会跳过知识库自动构建，对话主流程仍可运行。
- `星穹铁道—风堇（含武器）_by_崩坏：星穹铁道_f83f23e3cbce69f9982d3589847ccf01` 为预留的 3D 模型/贴图素材目录，用于未来前端表现开发；当前 release 包暂不包含该目录。
- 除上述素材外，项目代码均为原创开发。
