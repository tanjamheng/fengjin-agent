...
如果你有建议、发现了 bug——欢迎提交 Issue，或者只是想聊聊星铁和AI技术，欢迎在相关社区找到我：[Bilibili](https://space.bilibili.com/645659672)
...
设置页中的“启用心智”统一控制长期记忆、情绪与羁绊。心智模型与主对话模型各自独立调用，但可以填写同一套 API Key、Base URL 和模型；关闭心智后，已有记忆和状态会保留并冻结。
...
用户输入
  ...
  → 上下文组装（心智状态与记忆 → 角色锚点 → 滑动窗口）
  → LLM 流式生成（支持 Tool Calling，最多 5 轮）
  → 流式输出 → 角色漂移检测 → 会话持久化
  → 后台心智模型异步执行记忆提取与情绪/羁绊状态分析
```

### 情绪状态机

风堇不是每轮从零开始的 NPC——她有一个跨会话持续演化的情绪系统。底层用 PAD 三维模型（Pleasure-Arousal-Dominance）追踪心情底色：每轮主回复完成后，独立心智模型在后台分析最近三轮自然对话，以严格 JSON 给出状态目标；后端再用 EMA 指数移动平均平滑更新（α=0.3），防止单轮剧烈跳变。主模型因此只专注于扮演和回答。长时间不说话时，正向情绪衰减比负向慢——她会自然回归她的温暖底色。状态跨会话 JSON 持久化，重启不丢失。

### 羁绊系统

风堇和灰宝的关系是活的。四维羁绊模型（温暖度/信任度/正式度/幽默度）追踪两人关系的自然演变：后台心智模型结合最近对话与当前关系输出目标值，后端计算隐含变化量，经 change clamp（单轮 ±0.05 封顶）和接近度衰减（越亲近越难更进一步）后累加。信任几乎锁定（180 天半衰期），温暖会慢慢降温（14 天），久不联系关系自然生疏——但不会失忆。"关系可变，角色核心不可变。"
...
每次对话结束后，心智模型的记忆任务异步提取值得记住的事实，经 PII 过滤和向量去重后，按三级阈值路由：太相似的丢弃，明显不同的直写，中间模糊的交给模型判断合并。记忆任务与状态任务共享一套 API 配置，但调用、提示词和失败边界彼此独立，均不阻塞主对话。写入通过单线程队列串行化（ChromaDB 非线程安全）。检索时双层并行：`data/memory/core_memory.md` 全文读取（~1ms）+ ChromaDB 语义搜索（~30ms）。该文件包含用户私有记忆，默认不入 Git。
...
双击即开，全程零命令行。左边角色展示区始终显示，右边加载区展示两阶段进度：**预处理**（自动扫描缺失模型→下载→FP16 量化→构建知识库，每步有进度反馈，15 分钟无响应弹重试）→ **系统加载**（初始化安全、心智、角色漂移与 RAG 等组件，5 分钟无响应弹重试）。出错可重试/跳过。加载完成淡入聊天界面。启动时自动暖机检测 GPU 显存 → 按优先级分配模型（P0 嵌入 > P1 重排序 > P2 安全护栏），显存不够自动降级 CPU；系统内存不足时自动减载保护。
...
| 类别 | 技术 |
|------|------|
| LLM | OpenAI 兼容协议，主对话 + 后台心智（记忆与状态双调用） |
